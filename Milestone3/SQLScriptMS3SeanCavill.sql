...
CREATE PROCEDURE CreateDataBase()
	...
	CREATE TABLE tblplayer (
      ...
      CONSTRAINT NameLength CHECK (CHAR_LENGTH(`userName`) >= 3),
      CONSTRAINT PasswordLength CHECK (CHAR_LENGTH(`password`) >= 5)
	  );
...
INSERT INTO tblPlayer(`userName`, `email`, `password`, `loginAttempts`, `isLocked`, `isAdministrator`, `isOnline`)
  Values('Admin', 'Admin@gmail.com','Password', 0, false, true, false),
		('Test1', 'test1@gmail.com','Password1', 0, false, false, false),
		('Test2', 'test2@gmail.com','Password2', 0, true, false, false);

INSERT INTO tblMap(`Name`, `xLength`, `yLength`, `xHome`, `yHome`)
Values('Small', 15, 7, 8, 4),
	  ('Medium', 21, 11, 11, 6),
      ('Large', 33, 17, 17, 8);        
/*
...
BEGIN
START TRANSACTION;
	SELECT * FROM tblPlayer AS MESSAGE
...
START TRANSACTION;
	IF EXISTS (SELECT `userName` FROM tblPlayer WHERE `userName` = pUserName AND `isAdministrator` = true) THEN
		BEGIN
			SELECT ("Admin") AS MESSAGE;
		END;
	ELSE
	BEGIN
		SELECT ("User") AS MESSAGE;
	END;
    END IF;
    COMMIT;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `EditPlayer`(pUserName varchar(20), pNewEmail varchar(50), pNewPassword varchar(25), pNewAdminStatus bit, pNewLockedStatus bit)
BEGIN
START TRANSACTION;
...
IF EXISTS(SELECT `userName` FROM tblPlayer WHERE `userName` = pUserName) THEN
	...
	ELSE
		UPDATE tblPlayer
		SET `email` = pNewEmail, `password` = pNewPassword, `isAdministrator` = pNewAdminStatus, `isLocked` = pNewLockedStatus
		WHERE `userName` = pUserName;
		SELECT ("Success") AS MESSAGE;
		END IF;
	END;
ELSE
	INSERT INTO tblPlayer(`userName`,`email`, `password`, `isAdministrator`, `isLocked`)
    VALUES(pUserName, pNewEmail, pNewPassword, pNewAdminStatus, pNewLockedStatus);
    SELECT ("Success") AS MESSAGE;
END IF;
COMMIT;
...
IF EXISTS(SELECT `userName` FROM tblPlayer WHERE `userName` = pUserName) THEN
	BEGIN
		...
		SELECT concat("Deleted") AS MESSAGE;
	END;
	...
	IF EXISTS(SELECT `name` FROM tblMap WHERE `name` = pMapName) THEN
			...
			WHILE  lcYCount <= lcYLength DO
				WHILE lcXCount <= lcXLength DO
				   ...
				   VALUES(pGameName, lcXCount, lcYCount);
...
DECLARE lcBombStatus varchar(10);
SET lcBombStatus = "";
...
IF ((ABS(lcCurrentTileX-pXValue) <= 1) AND (ABS(lcCurrentTileY-pYValue) <= 1)) THEN
	...
	IF NOT EXISTS(SELECT `GameName`, `TileXValue`, `TileYValue`, `isActive`, COUNT(*) FROM tblGamePlayer
	 ...
	 SELECT concat(pPlayerName, ' Moved successfully', lcBombStatus) AS MESSAGE;
...
DELIMITER ;
/* needed to add in ms3 for functionality */
DROP PROCEDURE IF EXISTS TileVisibility;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `TileVisibility`(pGameName varchar(20))
BEGIN
SELECT `xValue`, `yValue`, `isVisible`, `number` FROM tbltile  AS MESSAGE
WHERE `gameName` = pGameName;
END;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `TileItems`(pGameName varchar(20))
BEGIN
SELECT `xValue`, `yValue` FROM tbltileItem  AS MESSAGE
WHERE `gameName` = pGameName;
END;;
DELIMITER ;


DROP PROCEDURE IF EXISTS TileBombs;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `TileBombs`(pGameName varchar(20))
BEGIN
SELECT `xValue`, `yValue`, `damage` FROM tbltilebomb  AS MESSAGE
WHERE `gameName` = pGameName;
END;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `ConsumeItem`(pPlayerName VARCHAR(20), pGameName VARCHAR(20), pItemName VARCHAR(10))
BEGIN
...
SELECT `userName`, `score`, `colour`, `tileXValue`, `tileYValue` FROM tblGamePlayer AS MESSAGE
WHERE `isActive` = true AND `gameName` = pGameName
ORDER BY `score` DESC;
...
DROP PROCEDURE IF EXISTS GetStartingPosition;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetStartingPosition`(pPlayerName varchar(20), pGameName varchar(20))
BEGIN

SELECT `colour`, `tileXValue`, `tileYValue` FROM tblGamePlayer AS MESSAGE
WHERE `userName` = pPlayerName AND `gameName` = pGameName;
END ;;

DELIMITER ;
DROP PROCEDURE IF EXISTS MapSize;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `MapSize`(pGameName varchar(20))
BEGIN

DECLARE lcMap varchar(6);

SELECT `mapName` FROM tblGame
WHERE `name` = pGameName
INTO lcMap;

SELECT `name`, `xLength`, `yLength`, `xHome`, `yHome` FROM tblmap AS MESSAGE
WHERE name = lcMap;
